...
#define kSTAlertPaddingV     10
#define kSTAlertPaddingH     10
...
- (void)loadUI{
    ...
    //设置的整个backgroudVIew的颜色
    _backgroundView.backgroundColor = [UIColor colorWithRed:0/255.0f green:0/255.0f blue:0/255.0f alpha:0.6];
    ...
    _backgroundView.layer.borderWidth = 2.0;
...
- (void)prepareForShow{
    ...
    _maxAlertViewHeight = [UIScreen mainScreen].bounds.size.height-200;
    [self loadTitle];
...
- (void)loadTips{
      ...
      UILabel *lblTips  = [UILabel new];
        lblTips.textColor = [UIColor whiteColor];
        lblTips.font = [UIFont boldSystemFontOfSize:14];
        lblTips.textAlignment = NSTextAlignmentCenter;
        lblTips.numberOfLines = 0;
    lblTips.text = @"稍后会发放到您的钱包中!";
  
 
    [self addLabel:lblTips maxHeight:100000];
}
