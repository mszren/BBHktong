...
//HeightScale 是占整个view的高度的比例(0.0 -> 1.0)  added by xdyang
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration HeightScale:(CGFloat)scale
{
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToast:toast duration:duration HeightScale:scale];
}

- (void)makeAttributedToast:(NSAttributedString *)message duration:(NSTimeInterval)duration HeightScale:(CGFloat)scale
{
    UIView *toast = [self viewForAttributedMessage:message title:nil image:nil];
    [self showToast:toast duration:duration HeightScale:scale];
}
...
//added by xdyang
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration HeightScale:(CGFloat)scale
{
    ...
    if (y == 0)
    {
        y += CGRectGetHeight(toast.bounds)/2;
    }
    if(y == height)
    {
        y -= CGRectGetHeight(toast.bounds)/2;
    }
    CGPoint center = CGPointMake(toast.center.x, y);
    toast.center = center;
...
- (CGSize)sizeForAttributedString:(NSAttributedString *)string font:(UIFont *)font constrainedToSize:(CGSize)constrainedSize lineBreakMode:(NSLineBreakMode)lineBreakMode {
    if ([string respondsToSelector:@selector(boundingRectWithSize:options:context:)]) {

        CGRect boundingRect = [string boundingRectWithSize:constrainedSize options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) context:nil];
        return CGSizeMake(ceilf(boundingRect.size.width), ceilf(boundingRect.size.height));
    }
    return CGSizeZero;
}
...
- (UIView *)viewForAttributedMessage:(NSAttributedString *)attributedMessage title:(NSString *)title image:(UIImage *)image {
    // sanity
    if((attributedMessage == nil) && (title == nil) && (image == nil)) return nil;
    ...
    if (attributedMessage != nil) {
        ...
        // size the message label according to the length of the text
        CGSize maxSizeMessage = CGSizeMake((self.bounds.size.width * CSToastMaxWidth) - imageWidth, self.bounds.size.height * CSToastMaxHeight);
        CGSize expectedSizeMessage = [self sizeForAttributedString:attributedMessage  font:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];
        messageLabel.frame = CGRectMake(0.0, 0.0, expectedSizeMessage.width, expectedSizeMessage.height);
    }
