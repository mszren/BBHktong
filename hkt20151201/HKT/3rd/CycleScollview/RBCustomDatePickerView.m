...
@interface RBCustomDatePickerView()
{
    ...
    UIButton                    *selectTimeIsNotLegalLabel;//所选时间是否合法
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        _nowPickerShowTimeStr = [[NSString alloc] init];
        _nowPickerShowTimeNumberStr = [[NSString alloc] init];
...
//    nowPickerShowTimeLabel = [[UILabel alloc] initWithFrame:CGRectMake(17.0, 117.0, 278.5, 18)];
    _nowPickerShowTimeLabel = [[UILabel alloc] initWithFrame:CGRectMake(20.0, 117.0, 278.5, 18)];
//    [nowPickerShowTimeLabel setBackgroundColor:[UIColor blueColor]];
    [_nowPickerShowTimeLabel setFont:[UIFont systemFontOfSize:18.0]];
    [_nowPickerShowTimeLabel setTextColor:RGBA(248, 248, 255, 1)];
    [_nowPickerShowTimeLabel setTextAlignment:NSTextAlignmentCenter];
...
//    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@年%d月%d日 %@ %@:%@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,weekString,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)],[dateString substringWithRange:NSMakeRange(12, 2)]];
    _nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@年%d月%d日 %@ %@:%@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,weekString,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)],[dateString substringWithRange:NSMakeRange(12, 2)]];
    _nowPickerShowTimeStr = [NSString stringWithFormat:@"%@-%d-%d  %@:%@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)],[dateString substringWithRange:NSMakeRange(12, 2)]];
    
    NSDate *nowDate = [NSDate date];
    NSString *nowString = [dateFormatter stringFromDate:nowDate];
    NSDate *nowStrDate = [dateFormatter dateFromString:nowString];
    _nowPickerShowTimeNumberStr = [NSString stringWithFormat:@"%d", (long)[nowStrDate timeIntervalSince1970]];

//    NSLog(@"nowPickerShowTimeLabel.text == %@",_nowPickerShowTimeLabel.text);
    //时间label
    
//    [self addSubview:_nowPickerShowTimeLabel];
    timeBroadcastView = [[UIView alloc] init];
    if ([[UIScreen mainScreen] bounds].size.width == 375) {
      timeBroadcastView.frame = CGRectMake(50, [[UIScreen mainScreen] bounds].size.height / 2 - 80, [[UIScreen mainScreen] bounds].size.width - 100, 200.0);
        ...
        timeBroadcastView.frame = CGRectMake(70, [[UIScreen mainScreen] bounds].size.height / 2 - 150, [[UIScreen mainScreen] bounds].size.width - 140, 200.0);
    }
//    timeBroadcastView = [[UIView alloc] initWithFrame:CGRectMake(70, [[UIScreen mainScreen] bounds].size.height / 2 - 100, [[UIScreen mainScreen] bounds].size.width - 140, 200.0)];
    ...
    [middleSepView setBackgroundColor:RGBA(65.0, 168.0, 236.0, 1.0)];
    ...
    selectTimeIsNotLegalLabel = [[UIButton alloc] initWithFrame:CGRectMake(17, 339.5, 278.5, 40)];
...
//    [selectTimeIsNotLegalLabel setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
//    [selectTimeIsNotLegalLabel setTitle:@"完成" forState:UIControlStateNormal];
//    selectTimeIsNotLegalLabel.titleLabel.font = [UIFont systemFontOfSize:18];
//    [selectTimeIsNotLegalLabel setBackgroundColor:[UIColor colorWithRed:14/255.0f green:141/255.0f blue:219/255.0f alpha:1]];
//    [selectTimeIsNotLegalLabel addTarget:self action:@selector(selectTimeIsNotLegalLabel) forControlEvents:UIControlEventTouchUpInside];
//    
//    
...
- (void)scrollviewDidChangeNumber
{
    ...
    NSString *weekString = [self fromDateToWeek:dateString];
    _nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%d年%d月%d日 %@ %02d:%02d:%02d",yearInt,monthInt,dayInt,weekString,hourInt,minuteInt,secondInt];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    ...
    NSDate *selectDate = [dateFormatter dateFromString:selectTimeString];
      ...
      _nowPickerShowTimeStr  =  selectTimeString;
    
//    NSLog(@"selectTimeString == %@",selectTimeString);
